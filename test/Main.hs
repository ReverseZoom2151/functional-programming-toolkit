...
sudokuTests = case parseBoard easyPuzzle of
  ...
  Right board -> do
    ...
    roundTrip <- check "Sudoku export round-trips through the compact parser" (parseBoard (exportBoard board) == Right board)
    rating <- check "Sudoku difficulty analysis reports solver evidence" (let report = analyseDifficulty board in difficulty report == Beginner && emptyCells report == 51 && searchNodes report > 0)
    pure (solved && unique && malformed && inconsistent && noSolution && ambiguous && hint && roundTrip && rating)
  where
